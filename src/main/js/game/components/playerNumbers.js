define(require => {
  ...
  function enable() {
    ...
    return cards.map(async card => {
      ...
      // For this, we need to check winning numbers that have already started, but not necessarily finished revealing
      // Otherwise, we end up in a situation where a REVEAL ALL will mark some as gold, and some as silver, even though for
      // all intents and purposes, it's been revealed
      const colour = !card.matched && numberState.winningStart.includes(nextData[0]) ? 'Gold' : 'Silver';
      card.populate(nextData, colour);
  ...
  function checkMatch(data) {
    ...
    for (let i = 0; i < matchedCards.length; i++) {
      if (!autoPlay.enabled || data.isRevealAll) {
        ...
        if (matchedCards[i].uncovered){
          matchedCards[i].match();
          matchedCards[i].presentWin();
          // Set the text styles
          matchedCards[i].setValueTextWin();

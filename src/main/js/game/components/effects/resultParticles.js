define(require => {
  ...
  function initEmitters(inOrientation, inContainer, inConfig) {
    ...
    inContainer.emitter = utils.createEmitter(
      ...
      inConfig.bigWinSettings.isAnimated, {
        startFrame: inConfig.bigWinSettings.startFrame,
        endFrame: inConfig.bigWinSettings.endFrame,
        isSingleNumFormat: true,
        frameRate: inConfig.bigWinSettings.frameRate
      }
    );
  ...
  function toggleCoinShower(inVal) {
    if (!_containerPortrait || !_containerLandscape) {
      return;
    }
    //run all emitters
    if (_containerPortrait.emitter && _containerLandscape.emitter) {
